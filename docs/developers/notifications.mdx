...
<StepGuide steps={[
  {
    title: "Create a transaction with notification channels",
    description: <>Include the <code>notifications</code> parameter when creating the transaction via the <a href="/developers/payments/checkout-api">Checkout API</a>:<br /><br /><pre><code>{`{
  ...
  },
  {
    ...
    description: <>Use <code>session_id</code> or <code>order_no</code> to identify the transaction, and specify the channels:<br /><br /><pre><code>{JSON.stringify({"order_no": "example_order_no", "channels": ["sms", "email", "whatsapp"]}, null, 2)}</code></pre></>,
  },
  {
    title: "Handle the response",
    description: <>Successful notification returns:<br /><br /><pre><code>{JSON.stringify({"message": "Customer notified."}, null, 2)}</code></pre></>,
  },
  {
    title: "Resend as needed",
    description: <>You can resend notifications for the same or different payment states as many times as needed — just repeat step 2 with the same <code>session_id</code> or <code>order_no</code>.<br /><br /><strong>Supported states per channel:</strong><br />• <strong>Email &amp; SMS:</strong> <code>created</code>, <code>paid</code>, <code>canceled</code>, <code>failed</code>, <code>expired</code>, <code>authorized</code>, <code>voided</code>, <code>refunded</code>, <code>captured</code><br />• <strong>WhatsApp:</strong> <code>created</code>, <code>paid</code>, <code>canceled</code>, <code>failed</code>, <code>expired</code>, <code>authorized</code></>,
  },
