...
<StepGuide startIndex={0} steps={[
  {
    title: "Discover Payment Methods",
    description: <>Before creating a session, call the <a href="../payments/payment-methods">Payment Methods API</a> with <code>auto_debit: true</code> to discover which gateways support tokenization and auto-debit.<br /><br /><pre><code>{`POST /b/pbl/v2/payment-methods/
