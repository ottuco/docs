...
<StepGuide steps={[
  {
    title: "Retrieve pg_codes (optional)",
    description: <>Call the <a href="/developers/payments/payment-methods">Payment Methods API</a> to get available gateway codes, or use a known <code>pg_code</code> directly.</>,
  },
  {
  ...
  },
  {
    ...
    description: <><strong>Response</strong> (key fields):<br /><br /><pre><code>{`{
  "amount": 5.815,
  "checkout_url": "${OTTU_CONNECT_BASE_URL}/b/checkout/redirect/start/?session_id=...",
  ...
  },
  {
    ...
    image: "/img/developers/invoices/invoice-sample.png",
    imageAlt: "Sample invoice",
  },
  {
    title: "Send the payment link",
    description: <>Share the <code>checkout_url</code> with the customer. They can view the invoice, download the PDF, and pay.</>,
  },
  {
    title: "Handle the webhook",
    description: <>Ottu posts the transaction result to your <a href="/developers/webhooks/payment-events">webhook_url</a>. Use the <code>session_id</code> to match the response to the original invoice.</>,
  },
]} nextSectionId="use-cases" />
