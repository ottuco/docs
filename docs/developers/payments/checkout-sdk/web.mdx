...
import CSPGenerator from "@site/src/components/CSPGenerator";
...
Ensure that you utilize the public key and refrain from using the [private key](../../../getting-started/authentication#api-key-auth). The private key should remain confidential at all times and must not be shared with any clients.
:::
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-display-mode-grid.png" alt="Web SDK grid display mode" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-display-mode-column.png" alt="Web SDK column display mode" /><figcaption></figcaption></figure>
...
<Tabs>
  <TabItem value="sample" label="Sample theme" default>
    ...
    <figure><img src="/img/developers/checkout-sdk/web/web-sdk-theme-sample.png" alt="Web SDK sample theme" width="563" /><figcaption></figcaption></figure>
  ...
  <TabItem value="dark" label="Dark theme">
    ...
    Checkout.init({
      selector: "checkout",
      merchant_id: 'ksa.ottu.dev', // use your merchant id
    ...
    <figure><img src="/img/developers/checkout-sdk/web/web-sdk-theme-dark.png" alt="Web SDK dark theme" /><figcaption></figcaption></figure>
  ...
  <TabItem value="minimal" label="Minimal theme">
    ...
    <figure><img src="/img/developers/checkout-sdk/web/web-sdk-theme-minimal.png" alt="Web SDK minimal theme" width="563" /><figcaption></figcaption></figure>
  ...
  <TabItem value="side-by-side" label="Side By Side Buttons">
    ...
    <figure><img src="/img/developers/checkout-sdk/web/web-sdk-theme-side-by-side.png" alt="Web SDK side-by-side buttons theme" /><figcaption></figcaption></figure>
  ...
  <TabItem value="hide-header" label="Hide SDK Header">
    ...
    <figure><img src="/img/developers/checkout-sdk/web/web-sdk-theme-hide-header.png" alt="Web SDK with hidden header" width="563" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-scenario-hide-amount.png" alt="Web SDK with hidden amount" width="563" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-scenario-button-type.png" alt="Web SDK button type customization" width="563" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-scenario-button-color.png" alt="Web SDK button color customization" width="563" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-supported-values-01.png" alt="Supported theme CSS values part 1" /><figcaption></figcaption></figure>

<figure><img src="/img/checkout-sdk/image13%20%281%29.png" alt="Supported theme CSS values part 2" /><figcaption></figcaption></figure>

<figure><img src="/img/developers/checkout-sdk/web/web-sdk-supported-values-03.png" alt="Supported theme CSS values part 3" /><figcaption></figcaption></figure>
...
<figure><img src="/img/checkout-sdk/Apple%20Pay%20button.png" alt="Apple Pay button in Web SDK" /><figcaption></figcaption></figure>
...
<figure><img src="/img/checkout-sdk/2480DAEF-F1E6-47CE-A271-418F222A0BBD.jpg" alt="KNET payment failure popup" /><figcaption></figcaption></figure>
...
<figure><img src="/img/checkout-sdk/Google%20Pay%20Button.png" alt="Google Pay button in Web SDK" /><figcaption></figcaption></figure>

### STC Bank

If you have completed the STC Bank integration between Ottu and STC Bank, the Checkout SDK will handle the necessary checks to display the STC Bank button seamlessly. When you initialize the Checkout SDK with your [session_id](../../checkout-api) and payment gateway codes [pg_codes](../../checkout-api), the SDK will automatically verify the following conditions:

1. The `session_id` and `pg_codes` provided during SDK initialization must be associated with the STC Bank Payment Service. This ensures that the STC Bank option is available for the customer to choose as a payment method.
2. The Web SDK displays the STC Bank button irrespective of whether the customer has provided a mobile number while creating the transaction or not.

#### Customize Button

You can customize the STC Bank button using the Checkout SDK's [formsOfPayment](#formsofpayment-array) and [theme](#theme-object) properties. The `formsOfPayment` property tells the Checkout SDK to render only the STC Bank button. If you don't include this property, the SDK will render all available payment options.
...
<figure><img src="/img/checkout-sdk/stc%20pay%20Button.png" alt="STC Bank button in Web SDK" /><figcaption></figcaption></figure>
...
For full balance behavior, partial-payment rules, and the four-hour reservation auto-release, see the [Wallet section](/developers/payments/wallet/).
...
Onsite Checkout lets customers enter their card details directly on your page, without being redirected to a separate payment gateway page. The card form is rendered inside a PCI-compliant iframe hosted by Ottu's **Jazz SDK** and embedded inline within the Checkout SDK container, so the entire payment flow — card entry, 3D Secure challenge, and final result — happens on the merchant page.

### How to enable

Onsite Checkout is loaded when `'jazz'` is part of the [`formsOfPayment`](#formsofpayment-array) array passed to [`Checkout.init`](#checkoutinit). Whether you need to change anything depends on whether you currently pass `formsOfPayment` at all:

- **If you don't pass `formsOfPayment`**, no change is required, by default all forms of payment are enabled, including `'jazz'`.
- **If you do pass `formsOfPayment` explicitly**, make sure `'jazz'` is included alongside your other entries. Forms of payment that are not listed will not be rendered.
...
Checkout.init({
  ...
  formsOfPayment: ["jazz", "tokenPay", "applePay"],
});
```

:::note

Onsite Checkout is enabled per merchant according to your available tier. To enable it for your account, contact [csd@ottu.com](mailto:csd@ottu.com).

If `'jazz'` is omitted, or if a payment method has not been enabled for onsite checkout on your account, the Checkout SDK falls back automatically to the standard redirect-to-gateway flow for that method.
:::

### Flow

From the customer's perspective, an onsite payment looks like this:

1. The customer selects a card-based payment method in the checkout.
2. The card form (rendered by the Jazz SDK) appears inline inside the checkout container.
3. The customer enters their card details and clicks **Pay**.
4. If the card issuer requires 3D Secure, the challenge runs in-flow without leaving the page.
5. When the payment finishes, the SDK invokes [`successCallback`](#successcallback), [`errorCallback`](#errorcallback), or [`cancelCallback`](#cancelcallback) depending on the outcome.
...
Please note that due to technical constraints associated with off-site redirection during the payment process, the `successCallback` and `cancelCallback` functions are only called for on-site checkouts. However, the `errorCallback` function is called for any kind of payments. On-site checkouts include options such as Apple Pay, Google Pay, payments with saved cards, and [onsite card form](#onsite-checkout) transactions, which support callback functionality for a seamless user experience.
...
In `tokenPay` and `jazz` payment methods, the SDK passes the card details to the `beforePayment` callback. This allows you to identify the card brand or specific bank (via PAN) and apply a discount dynamically.
...
When `tokenPay` or `jazz` is used, the `paymentData` argument in the hook will contain the following `card` object. The same shape is also delivered for [Onsite Checkout](#onsite-checkout) payments, so any logic written against this structure works for both the gateway-redirect and onsite flows.
...
<figure><img src="/img/checkout-sdk/_-%20visual%20selection%20%281%29.png" alt="validatePayment hook flow diagram" width="375" /><figcaption></figcaption></figure>
...
<figure><img src="/img/checkout-sdk/image%20%2839%29.png" alt="Success popup notification" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-popup-error.png" alt="Error popup notification" /><figcaption></figcaption></figure>
...
<figure><img src="/img/developers/checkout-sdk/web/web-sdk-popup-redirect.png" alt="Redirect popup notification" /><figcaption></figcaption></figure>
...
When you embed the Checkout SDK on your site, you need a Content Security Policy that allows the SDK to load its scripts, open payment iframes, and call Ottu's APIs. The generator below builds a ready-to-paste policy based on your environment and the payment methods you plan to offer.

<CSPGenerator />

:::note How origins are determined
The base Ottu entries (`assets.ottu.net`, `*.ottu.net`, `*.ottu.com`) are always required — they cover the SDK script, payment iframes, and API calls. Each Form of Payment can add extra origins (for example, Apple Pay adds `applepay.cdn-apple.com` and the Apple Pay gateway endpoints). Toggle the FOPs you're enabling to see only the entries you actually need.
:::
...
<FAQ>
  ...
  <FAQItem question="5 Are there any known compatibility issues with browsers or platforms?">
    ...
    - For and Google Pay, STC Bank & other payments methods, always refer to their official documentation for the most recent information about compatibility issues.
  </FAQItem>
