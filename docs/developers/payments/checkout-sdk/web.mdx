...
Include `'jazz'` in the `formsOfPayment` array when calling [`Checkout.init`](#checkoutinit). The Jazz SDK will only be loaded when this value is present.

```javascript
Checkout.init({
  selector: "checkout",
  merchant_id: "example.ottu.com",
  apiKey: "<public-key>",
