...
<StepGuide steps={[
  ...
  },
  {
    ...
    description: <>Customer selects Wallet → SDK shows "10.000 KWD will be applied" → submits → only the session amount is deducted. Surplus stays in the M-Wallet.</>,
  ...
  },
  {
    ...
    description: <>When the session is configured for authorization-only, M-Wallet is not offered. M-Wallet supports immediate-capture flows only.</>,
  ...
  {
    title: "Try it",
    description: <>The live demo seeds a fresh M-Wallet for a generated customer_id and launches the SDK with M-Wallet enabled. Use the demo on the <a href="/developers/payments/wallet/">M-Wallet overview page</a>.</>,
...
OttuCheckoutWidget(
  ...
  successCallback: (message) => _handleSuccess(message),
  cancelCallback: (message) => _handleCancel(message),
  errorCallback: (message) => _handleError(message),
),
