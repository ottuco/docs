...
2. **Then the points — automatically.** The gateway refund that leaves nothing owed through the gateway also queues the return of the redeemed points: within seconds Ottu reverses the redemption at Qitaf and books it, with no further click. The **whole** redeemed amount comes back in one step; there is no partial return.

For an order paid with **points only** there is no gateway half: the parent's ordinary **Refund** returns the points (the amount is shown locked to the full redeemed amount), and so does **Return Qitaf points** on the points leg. Both doors lead to the same single, idempotent return.
...
sequenceDiagram
    ...
    Note over O: nothing owed through the gateway any more — points return queued
    O->>Q: Reverse the redemption (30 SAR of points)
    Q-->>O: Reversed
    O-->>M: Operation webhook: refund 30 SAR, pg_response.code 0
```

:::note The points come back on their own — the API does not return them directly
The automatic return is triggered by the gateway refund itself, whichever way you made it (dashboard or Operations API), and follows the same refund permission and the same [maker-checker approval](/business/operations/two-step-authorization) as that refund. The dashboard additionally offers the return by hand — **Return Qitaf points** on the points leg, and the parent's own **Refund** once the points are all that is left — for points-only orders and for the rare case where Qitaf could not be reached at the time. The public [Operations API](../operations.md) has no call that returns loyalty points: a `refund` with `destination: "wallet"` is rejected on a points-funded order, exactly as before. What your integration *does* see is the outcome, through the webhooks below.
:::
...
| Rule | Behaviour |
| --- | --- |
| **Order** | Gateway money first, points second. Until the gateway portion is fully refunded nothing returns the points: the automatic return is not queued, the manual actions are not offered, and a direct request is refused with the remaining gateway amount. A refund that is still pending approval counts as outstanding. |
| **Trigger** | The gateway refund that leaves nothing owed through the gateway queues the return automatically. A points-only order is returned through the parent's Refund (or the leg's own action). |
| **Amount** | Always the full redeemed amount. Nothing to enter, nothing partial. |
| **Once** | A second return — by hand, by a retry, or by the automatic run landing after a manual one — is refused; two simultaneous requests book exactly one. |
| **Recoverable** | If Qitaf times out or answers with a retryable error, nothing is booked; Ottu retries on its own every few minutes (for a bounded number of attempts, then escalates to Ottu operations), and the dashboard actions stay available to return the points by hand meanwhile. If Qitaf answers that the redemption can no longer be reversed (for example the reversal window has passed, or the redemption is unknown to it), the actions are withdrawn and the operator is told to refund the customer another way. |
