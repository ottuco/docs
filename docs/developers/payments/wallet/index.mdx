...
M-Wallet (merchant wallet) lets you refund customer payments to a stored balance keyed by merchant, customer, and currency, then let the customer spend that credit at any future Ottu checkout in the same currency. No PII is stored on the wallet service.
  ...
  "payment_methods": [
    {
      ...
      "name": "Ottu Sandbox",
      "connector": {
        "identifier": "ottu_sandbox",
        "processor": "ottu_sandbox"
      },
      "is_sandbox": true,
...
For the full request/response reference, headers, error codes, and supported filters — including the [`connector`](../payment-methods#api-reference) object that identifies each method's gateway and processor — see the [Payment Methods API page](../payment-methods).
...
Stacked-wallet setups (e.g., M-Wallet + Qitaf) emit one record per provider in the same payload; single-wallet payments emit a one-element list — so you can reconcile multi-wallet payments without polling the wallet read APIs.
...
- [Refund to M-Wallet (Operations)](../../operations#refund-to-wallet) — the refund API change that creates wallet credits.
- [Checkout SDK — M-Wallet section](../checkout-sdk/web#ottu-wallet) — how wallet appears in the SDK.
