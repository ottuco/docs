...
<FAQ>
  ...
  <FAQItem question="Can I charge a wallet without the Checkout SDK?">
    Yes — through [One-Step Checkout](../checkout-api#one-step-checkout). Call the Checkout API with `payment_instrument.instrument_type: "wallet"` to debit a customer's wallet server-to-server in a single request, no SDK required. Include `wallet_code` in the payload only when the gateway has more than one wallet configured. If you're using the regular session-then-pay flow, the wallet appears in the Checkout SDK as described above.
  </FAQItem>
