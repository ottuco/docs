...
Try the wallet flow below. Each step shows the live API request and response — discover the wallet-capable pg_code, seed a fresh customer's wallet through the docs backend, create the Checkout session, then pay with wallet in the embedded SDK. Screenshots of the merchant-side flow are on the [business wallet pages](/business/wallet/).
