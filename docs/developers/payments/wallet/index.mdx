...
Use the [Payment Methods API](../payment-methods) (`POST /b/pbl/v2/payment-methods/`) to discover available methods for a session. When the customer has positive wallet balance in the session currency, a method with `type: "wallet"` appears alongside the other available methods.

```json title="POST /b/pbl/v2/payment-methods/"
{
  "plugin": "e_commerce",
  "operation": "purchase",
  "currencies": ["KWD"],
...
The `customer_id` parameter on the Payment Methods API is required for wallet discovery — it tells Ottu which customer's wallet balance to check. Support for this parameter is part of Redmine subticket 150892. Until that ships, wallet methods are surfaced automatically when you initialize the [Checkout SDK](../checkout-sdk/) with a `customer_id` on the session — you can skip the explicit discovery call and rely on the SDK to render the method when balance is positive.
:::
