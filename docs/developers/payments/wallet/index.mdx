...
### Wallet details in payment webhooks

When a wallet leg debits a session, the payment webhook delivered to your `webhook_url` carries the full wallet activity in a top-level [`customer_wallet_transactions`](../../webhooks/payment-events#response-payment-webhook-customer_wallet_transactions) array — one entry per wallet provider that contributed funds. Each entry includes the amount charged, the provider's `operation_id`, the ISO 4217 currency code, creation and modification timestamps, and the verbatim wallet-service response (status, `payment_entry_id`, `reserve_entry_id`). Stacked-wallet setups (e.g., Ottu wallet + Qitaf) emit one record per provider in the same webhook; single-wallet payments emit a one-element list — use it to reconcile multi-wallet payments without polling the wallet APIs.
