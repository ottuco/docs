...
|  `samsung_pay`  | Samsung Pay digital wallet              | Samsung Pay supported devices                                                             |
...
|    `wallet`     | Ottu wallet balance                     | Spending a customer's [wallet](/developers/payments/wallet/) credit server-to-server (no SDK). The `payload` carries `wallet_code` — required only when the gateway has more than one wallet configured. |
