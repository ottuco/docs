...
Ottu sends a [payment notification](../webhooks/payment-events.md) webhook to the `webhook_url` you provided in the API request. The webhook contains details like `status`, `amount`, `currency_code`, and `pg_response`. Ensure your server verifies and processes these webhook notifications.
