...
By default the Sandbox stores a minimal stub (`{"card_number": "…"}`) as the gateway response. When you need your integration to handle a **real gateway's response shape** — the exact JSON that **KNET, MPGS, CyberSource, Ottu PG, or MyFatoorah** returns — the hosted pay page can **mimic** it. This lets you exercise your webhook parsing, reconciliation, and gateway-specific handling against production-shaped payloads without ever calling the live gateway.
...
2. On the Sandbox pay page, under **"Mimic PG Response" (optional)**, click the logo of the gateway you want to imitate — **KNET, MPGS, CyberSource, Ottu PG, or MyFatoorah**. (Click the active logo again to clear the selection and fall back to the default stub.)
...
**Simulating a failed capture.** Add `sandbox_result` to the operation's `extra` ([Mechanism C](#how-outcome-control-works)). On `failed`/`error` the capture is rejected, **no child transaction is created**, and the parent stays `authorized`:
...
**Simulating failure / error.** Wallet payments carry no card and no per-call `extra`, so the outcome is read from the **transaction's `extra`** ([Mechanism C](#how-outcome-control-works)) — set it when you create the session:
...
- **Refund always succeeds** — the Sandbox cannot simulate a declined or rejected refund. Capture, void and wallet payments *can* be made to fail (via `sandbox_result`); refund cannot.
- **No 3-D Secure (3DS) simulation** — the Sandbox does not present a 3DS challenge or redirect; the outcome is decided directly by the card / token / `sandbox_result`.
- **Mimic only changes the response shape** — selecting a gateway under [Mimic PG Response](#scenario-2--mimic-a-real-gateways-response-shape) does not change the outcome (the card still decides) and the mimicked payload is stored verbatim, not re-processed.
- **The asynchronous `sandbox_inquiry_result` scheduler runs on non-production only.** On a production-flagged server, sessions created with `sandbox_inquiry_result` + `expires_in_minutes` will not auto-resolve. Use a dev/UAT environment.
- **`extra` does not drive interactive card outcomes** — for hosted checkout, tokenization, and auto-debit the result is set by the card/token. `extra` (`sandbox_inquiry_result`, `sandbox_result`) only drives the inquiry, capture, void and wallet paths.
